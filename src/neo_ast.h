...
neo_static_assert(UNOP__COUNT <= 255);
...
neo_static_assert(BINOP__COUNT <= 255);
...
#undef _
neo_static_assert(ASTNODE__COUNT <= 64);
...
#define ASTNODE_EXPR_MASK (ASTNODE_LITERAL_MASK|astmask(ASTNODE_UNARY_OP)|astmask(ASTNODE_BINARY_LIT)|(ASTNODE_GROUP)) /* All expression types. */
...
#define ASTNODE_CONTROL_FLOW (astmask(ASTNODE_BRANCH)|astmask(ASTNODE_RETURN)|astmask(ASTNODE_WHILE)|astmask(ASTNODE_BREAK)|astmask(ASTNODE_CONTINUE))

struct astnode_t {
    astnode_type_t type : 8;
    union {
        node_error_t n_error;
        node_group_t n_group;
        node_int_literal_t n_int_lit;
        node_float_literal_t n_float_lit;
        node_char_literal_t n_char_lit;
        node_bool_literal_t n_bool_lit;
        node_string_literal_t n_string_lit;
        node_ident_literal_t n_ident_lit;
