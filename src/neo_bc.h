...
/* Checked Instruction composition. */
static inline NEO_NODISCARD bci_instr_t bci_comp_mod1_imm24(opcode_t opc, imm24_t imm) {
    neo_assert(bci_fits_i24(imm), "24-bit signed imm out of range: %" PRIx32, imm); /* Verify immediate value. */
    neo_assert(opc_imms[opc] == IMM_I24, "Invalid imm mode for instruction"); /* Verify immediate mode. */
...
static inline NEO_NODISCARD bci_instr_t bci_comp_mod1_umm24(opcode_t opc, umm24_t imm) {
    neo_assert(bci_fits_u24(imm), "24-bit unsigned imm out of range: %" PRIx32, imm); /* Verify immediate value. */
    neo_assert(opc_imms[opc] == IMM_U24, "Invalid imm mode for instruction"); /* Verify immediate mode. */
...
static inline NEO_NODISCARD bci_instr_t bci_comp_mod1_no_imm(opcode_t opc) {
    neo_assert(opc_imms[opc] == IMM_NONE, "Invalid imm mode for instruction"); /* Verify immediate mode. */
